...
def PointDelta(point_a: exit_speed_pb2.Gps,
  ...
  return gps.EarthDistanceSmall((point_a.lat, point_a.lon), (point_b.lat, point_b.lon))

def PointDeltaFromTrack(track: base.Track, point: exit_speed_pb2.Gps) -> float:
  """Returns the distance in meters between two points."""
  return gps.EarthDistanceSmall((track.start_finish[0], track.start_finish[1]), (point.lat, point.lon))
